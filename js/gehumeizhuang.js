(function($){
  $(function(){
    // console.log($)
    // tab切换
    $('.jSortTab').on("mouseenter",'span',function(){
      $(this).addClass('current').siblings('.current').removeClass('current')
      $(this).parent().next().children().eq($(this).index()).addClass('current').siblings('.current').removeClass('current')
      var jSortLength = $(this).index();
      $(this).parent().prev().stop().animate({
        left:jSortLength*197.493
      })
    })
    ...
    $(document).on('scroll',function(){
      ...
      var dLayoutAreaTotal = $('.d-layout-area').length
      // 当滚动条的高度大于等于800的时候，让电梯导航显示出来
      if(_top>=800){
      ...
      // 鼠标滚动  让电梯导航添加类名
      for(var kk=4;kk<dLayoutAreaTotal-1;kk++){
        if(_top>=$('.d-layout-area').eq(kk).offset().top && _top<$('.d-layout-area').eq(kk+1).offset().top){
          $('.index-rbar li').eq(kk-4).addClass('active').siblings('.active').removeClass('active')
        }
      }
    })

    // 电梯导航  点击
    $('.index-rbar').on('click','li',function(){
      $(this).addClass("active").siblings('.active').removeClass('active')
      var dianTiIndex = $(this).attr('index')
      $('body,html').stop().animate({
        'scrollTop' : $('.d-layout-area').eq(dianTiIndex).offset().top
      },400)
    })
