...
$(function () {
	...
	$(function () {
		...
		$('.tp_add_cart').on('click', function () {
			...
			// 这里找到对应的 图片 介绍等
			var $url = $par.find('.t_prodimg img').attr('src')
			var $content = $par.find('.t_prodinfo .ti_name').text()
			var $price = $par.find('.t_prodinfo .tp_price b').text()
			// 找到购物车中的所有的包含店名字的元素
			var $shop_name = $order_content.parents('.cartBox').find('.shop_name a')
			// 找到列表中的商店名字,循环以便于逐个判断对应的列表对应的特定商店名字
